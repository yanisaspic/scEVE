...
get_default_hyperparameters <- function() {
  ...
  #' - clustering_methods: a vector of valid method names.
  #' Seurat, monocle3, SHARP and densityCut.
  #' 
  #' -> leftovers_strategy: a valid strategy to handle leftover cells.
  #' + default: leftover cells are in the leftover seed
  #' -> markers_strategy: a valid strategy to report marker genes.
  #' + default: markers are reported in a binary matrix. If marker i is over-represented in population j, it is 1.
  #'
  #' - figures_dir: a character. The name of the directory generated to store intermediate figures.
  #' - records_file: a character. The name of the file generated to store the results of the analysis.
  #' 
...
scEVE.iteration <- function(expression.init, population, records, params,
  ...
  while (TRUE) {
    ...
    clusterings <- get_clusterings(expression.init, data.loop, population,
    ...
    if (save) {write.xlsx(records, params$records_file, rowNames=TRUE)}
    break()
  }
